...
@if (Auth::user()->email == 'mahmoud.youssef@squadbcc.com' || Auth::user()->email == 'samer.tawfik@squadbcc.com' )
    {{-- All Users Projects --}}
    <a title="{{ __('All Users Projects') }}" class="logout"
        href="{{ route('all.users.projects') }}"><i class="fas fa-align-justify"></i></a>
@endif
@section('content')
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body modal-text">
                            ...
                            <a href="https://reveroplanner.com/"
                                class="btn btn-danger {{ app()->getLocale() == 'ar' ? 'float-left' : 'float-right' }} main-page-button"
                                data-dismiss="modal" aria-label="Close">{{ __('Close') }}</a>

                            <a href="https://reveroplanner.com/"
                                class="btn btn-rev {{ app()->getLocale() == 'ar' ? 'float-left' : 'float-right' }} main-page-button">{{ __('REVERO') }}</a>
                        </div>
                    </div>
                </div>
            </div>
            @foreach ($projects as $project)
                <div class="row" @if (app()->getLocale() == 'ar') style="direction:rtl" @endif>
                    ...
                    <div>
                        ...
                        <button type="submit" class="btn btn-rev float-left delete-btn" data-toggle="modal"
                            data-target="#exampleModal{{ $project->id }}"><i class="fas fa-trash-alt"></i></button>
                        <button type="button" class="btn btn-rev float-left  delete-btn share_button" data-toggle="modal"
                            data-target="#sharing{{ $project->id }}"><i class="fas fa-share-alt"></i></button>
                    </div>
                </div>
                <!-- Modal -->
                            ...
                            <div class="modal-body modal-text">
                                ...
                                <form action="{{ route('projects.destroy', [$project->id]) }}" method="post">
                                    ...
                                    <button type="submit"
                                        ...
                                        data-toggle="modal"
                                        data-target="#exampleModal{{ $project->id }}">{{ __('DELETE') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Sharing Modal -->
                <div class="modal fade" id="sharing{{ $project->id }}" tabindex="-1" role="dialog"
                    aria-labelledby="sharingModalLabel{{ $project->id }}" aria-hidden="true" style="color: black">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{ route('sharingLink.store', $project) }}" id="form{{ $project->id }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input readonly type="text"   id="myInput{{ $project->id }}"
                                            value="{{ route('study_info.view', [$code]) }}" class="form-control">
                            ...
                            <div class="modal-footer d-flex">
                                ...
                                <button type="submit" class="btn btn-info p-2" onclick="myFunction({{ $project->id }})" onmouseout="outFunc({{ $project->id }})"><span class="tooltiptext"><i
                                            class="far fa-copy" id="myTooltip{{ $project->id }}"> {{__('Copy')}}</i> </span>
                                </button>
                            </div>
                        </div>
                    </div>
