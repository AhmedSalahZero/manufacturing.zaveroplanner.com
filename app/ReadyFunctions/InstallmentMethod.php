...
class InstallmentMethod
{
	public function __calculate(
		...
		$downPaymentTwoDateFormatted =  Carbon::make($startDate)->addMonth($downPaymentTwoMonth)->format('Y-m-d');
		...
		for ($i = 1; $i <= $installmentCount; $i++) {
			$currentInstallmentDate = $downPaymentTwoDate->addMonths($installmentDuration)->format('Y-m-d');
